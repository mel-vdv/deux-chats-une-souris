...
app.get("https://jeu-de-mel.herokuapp.com/", (req, res, next) => {
    res.render('index', datasPug);
});
//-------------------------------
app.get('https://jeu-de-mel.herokuapp.com/connexion', (req, res, next) => {
    datasPug.texte = 'Entre ton pseudo : ';
    datasPug.url = 'https://jeu-de-mel.herokuapp.com/traitement-connexion';
    datasPug.valueSubmit = 'CONNEXION';
    res.render('formulaire', datasPug);
});
//-----------------------------------
app.get('https://jeu-de-mel.herokuapp.com/inscription', (req, res, next) => {
    datasPug.texte = 'Choisis un pseudo : ';
    datasPug.url = 'https://jeu-de-mel.herokuapp.com/traitement-inscription';
    datasPug.valueSubmit = 'INSCRIPTION';
    res.render('formulaire', datasPug);
});
//---------------------------------------
app.get('https://jeu-de-mel.herokuapp.com/menu', (req, res, next) => {
    datasPug.joueur = req.session.name;
    res.render('menu', datasPug);
});
//---------les scores : affichage ------------
app.get('https://jeu-de-mel.herokuapp.com/scores', (req, res, next) => {
...
//------------------------------------------
app.get('https://jeu-de-mel.herokuapp.com/play', (req, res, next) => {
    ...
    MongoClient.connect(urldb, { useUnifiedTopology: true }, (err, client) => {
        ...
        collek.find({}).toArray((er, dat) => {
            if (dat[0].joueur1 === req.session.name) {
                res.redirect('https://jeu-de-mel.herokuapp.com/jouer1');
            }
            else if (dat[0].joueur2 === req.session.name) {
                res.redirect('https://jeu-de-mel.herokuapp.com/jouer2');
            }
                ...
                res.redirect('https://jeu-de-mel.herokuapp.com/deconnect');
            }
        });
    });
});
//----------------------------------sessions----------------------------
app.get('https://jeu-de-mel.herokuapp.com/deconnect', (req, res, next) => {
    MongoClient.connect(urldb, { useUnifiedTopology: true }, (err, client) => {
        if (!err) {
            const collek = client.db('jeu').collection('partie');
            collek.find({}).toArray((er, dat) => {
...
app.post('https://jeu-de-mel.herokuapp.com/traitement-connexion', (req, res, next) => {
            ...
            collek.find({ pseudo: req.body.pseudo }).toArray((err, datass) => {
                if (!err) {
                    if (!datass.length) {
                        ...
                        res.redirect('https://jeu-de-mel.herokuapp.com/connexion');
                    }
                    else {
                        req.session.name = datass[0].pseudo;
                        req.session.points = datass[0].score;
                        res.redirect('https://jeu-de-mel.herokuapp.com/menu');
                    }
...
//------------------------------------
app.post('https://jeu-de-mel.herokuapp.com/traitement-inscription', (req, res, next) => {
    MongoClient.connect(urldb, { useUnifiedTopology: true }, (err, client) => {
        if (!err) {
            const collection = client.db('jeu').collection('users');
            collection.find({ pseudo: req.body.pseudo }).toArray((err, datas) => {
                if (!err) {
                    if (datas.length) {
                        ...
                        res.redirect('https://jeu-de-mel.herokuapp.com/inscription');
                    }
                    else {
...
//**********************************   LE JEU  !!!!   **************************************** */
app.get("https://jeu-de-mel.herokuapp.com/jouer1", (req, res, next) => {
    return res.sendFile(path.normalize(`${__dirname}/clientSocketio1.html`));
});
app.get("https://jeu-de-mel.herokuapp.com/jouer2", (req, res, next) => {
